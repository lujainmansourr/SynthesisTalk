...
async def analyze(data: dict):
    ...
    prompt = f"Please {action} the following text and use '•' to format key points:\n\n{content[:4000]}"
    ...
    try:
        ...
        # For "extract key points", force the bullet format
        if action == "extract key points":
            # Optionally, clean the reply and add bullets
            reply = '\n'.join([f"- {line.strip()}" for line in reply.splitlines() if line.strip()])
...
async def summarize_pdf(data: dict):
    ...
    if action == "summarize":
        prompt = f"Summarize the following text into a concise and well-structured paragraph:\n\n{content[:4000]}"
    elif action == "extract key points":
        prompt = f"Extract the key points from the following text as a list of clear, concise bullet points:\n\n{content[:4000]}"
    else:
    ...
    # 🔥 Format the text a bit for visual PDF output
...
async def chat(data: dict):
    ...
    session = sessions.get(session_id)
    if not session:
        session = sessions[session_id] = {"history": []}
